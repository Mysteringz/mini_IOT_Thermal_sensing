...
void drawRedBox(int start_x, int start_y, int x1, int x2, int y1, int y2, uint8_t resize) {
  ...
  for (int i = x1; i < x2; i++) {
    // BSP_LCD_DrawPixel(i, y1, red);
    // BSP_LCD_DrawPixel(i, y2, red);
    for (int tx = 0; tx < resize; tx++) {
      for (int ty = 0; ty < resize; ty++) {
        BSP_LCD_DrawPixel(start_x + i * resize + tx, start_y + y1 * resize + ty, red);
        BSP_LCD_DrawPixel(start_x + i * resize + tx, start_y + y2 * resize + ty, red);
      }
    }
  }
  for (int j = y1; j < y2; j++) {
    // BSP_LCD_DrawPixel(x1, j, red);
    // BSP_LCD_DrawPixel(x2, j, red);

    for (int tx = 0; tx < resize; tx++) {
      for (int ty = 0; ty < resize; ty++) {
        BSP_LCD_DrawPixel(start_x + x1 * resize + tx, start_y + j * resize + ty, red);
        BSP_LCD_DrawPixel(start_x + x2 * resize + tx, start_y + j * resize + ty, red);
      }
    }
  }
}
