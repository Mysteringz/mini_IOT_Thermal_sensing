...
tinyengine_status convolve_s8_kernel3_stride1_pad1_fpreq(const q7_t *input, const uint16_t input_x, const uint16_t input_y,
	...
	for (int i_out_y = 0; i_out_y < output_y; i_out_y++) {
		...
		for (int i_out_x = 0; i_out_x < output_x; i_out_x++) {
			...
			else if (base_idx_y == -1) { //pad the first row
			...
			{
				out = mat_mult_kernel_s8_s16_fpreq(kernel,
										  runtime_buf,
										  output_ch,
										  output_shift,
										  output_mult,
