...
//                            scale_y = exp(point_data[3]) * anchors[anchor_id][1];
                            ...
                            center_x = (sigmoid(point_data[0]) * 2 - 0.5 + x) * stride;
							center_y = (sigmoid(point_data[1]) * 2 - 0.5 + y) * stride;
							float w = sigmoid(point_data[2]) * 2;
							float h = sigmoid(point_data[3]) * 2;
							scale_x = w * w * anchors[anchor_id][0];
							scale_y = h * h * anchors[anchor_id][1];
...
det_box **postprocessing(signed char *input_data[3], signed char y_zero[3], float y_scale[3], signed char *data_buf,
