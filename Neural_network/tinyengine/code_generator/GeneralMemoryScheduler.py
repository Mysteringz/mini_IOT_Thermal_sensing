...
class GeneralMemoryScheduler:
    """
    A memory scheduler for TinyEngine that manages memory allocation for layers and tensors,
    optimizing memory usage for resource-constrained devices.

    Attributes:
        layer (list): List of layers in the model.
        tflite_op (bool): Whether to use TFLite operations.
        dummy_address (bool): Whether to use dummy addresses for tensors.
        memory_limit (int): Maximum memory limit for allocation.
        inplace (bool): Whether to enable in-place operations.
        outputTables (list): List of output tensors.
        mem_visual_path (str): Path for memory visualization output.
        VisaulizeTrainable (bool): Whether to visualize trainable tensors.
        sort_by_lifetime (bool): Whether to sort tensors by lifetime.
    """
    def __init__(
        self,
        layer,
        ...
        """
        Initialize the GeneralMemoryScheduler.
    ...
    def _isTranable(self, name):
        ...
        Returns:
            bool: True if the tensor is trainable, False otherwise.
        """
    ...
    def allocateMemory(self):
        """
        Allocate memory for tensors and layers, optimizing for in-place operations
        ...
        print("go through all tensors in the model")
        for i, op in enumerate(self.layer):
    ...
    def dumpLayerIndex(self):
        """
        Print the allocation details of each layer, including operator type,
        input index, and output index.
        """
        # header
    ...
    def dumpLayerMem(self):
        """
        Print detailed memory usage for each layer, including SRAM and Flash usage.
        """
        # header
        print(
    ...
    def __dumpMemInfo(self, layermem):
        """
        Print memory information for all layers.

        Args:
            layermem (list): List of memory usage details for each layer.
        """
        string = "-------Schedule-------|"
    ...
    def __sumKey(self, layers, key):
        """
        Sum the values of a specific key across all layers.
        ...
        Returns:
            int: Total sum of the key values.
        """
        result = 0
    ...
    def getBuffers(self):
        """
        Get the buffer sizes for different memory regions.

        Returns:
            dict: Dictionary of buffer sizes.
        """
        return self.buffers
    ...
    def profileResult(self):
        """
        Get the peak memory and Flash usage.

        Returns:
            tuple: Peak memory and Flash usage.
        """
        return self.peakmem, self.flash + self.bias + self.scale + int(self.code * 1024)

    def __increaseFlash(self, size):
        """
        Increase the Flash memory usage.

        Args:
            size (int): Size to add to Flash memory.
        """
        self.flash += int(size)

    def _enlargeBuffer(self, buf_str, size):
        """
        Enlarge a specific buffer to accommodate the given size.

        Args:
            buf_str (str): Name of the buffer.
            size (int): Size to enlarge the buffer to.
        """
