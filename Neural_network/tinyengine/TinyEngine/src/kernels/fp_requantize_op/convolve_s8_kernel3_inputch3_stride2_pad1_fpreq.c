...
tinyengine_status convolve_s8_kernel3_inputch3_stride2_pad1_fpreq(
	...
	// _PKHBT make (pack) two values place next to each other, but the operation is still happened independently
	// This is the pre processing for efficient parallel operation in later SIMD instructions
	...
	for (i_out_y = 0; i_out_y < output_y; i_out_y++) {
		for (i_out_x = 0; i_out_x < output_x; i_out_x++) {
			...
			// For a 3x3 kernel with 3 input channels, each sliding window contains 3x3x3 = 27 elements.
			two_column_buf += 27;
