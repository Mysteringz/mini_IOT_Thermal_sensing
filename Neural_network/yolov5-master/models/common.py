...
class Sum(nn.Module):
    """Weighted sum of explicitly specified layers https://arxiv.org/abs/1911.09070."""

    def __init__(self, indices, weight=False):
        """
        Initializes a module to sum outputs of layers with explicitly specified indices and optional weighting.
        Args:
            indices (list): List of layer indices to sum.
            weight (bool): Whether to apply learnable weights to the inputs.
        """
        super().__init__()
        self.indices = indices  # List of layer indices to sum
        self.weight = weight  # Apply weights boolean
        if weight:
            self.w = nn.Parameter(-torch.arange(1.0, len(indices)) / 2, requires_grad=True)  # Layer weights

    def forward(self, x, outputs):
        """
        Processes input through a customizable weighted sum of specified layers.
        Args:
            x (torch.Tensor): Current layer's input (not used in this case).
            outputs (list): List of outputs from previous layers.
        Returns:
            torch.Tensor: Result of the weighted or unweighted sum of specified layers.
        """
        y = outputs[self.indices[0]]  # Start with the first specified layer
        if self.weight:
            w = torch.sigmoid(self.w) * 2  # Normalize weights to the range [0, 2]
            for i, idx in enumerate(self.indices[1:]):  # Iterate over the remaining indices
                y = y + outputs[idx] * w[i]
        else:
            for idx in self.indices[1:]:  # Iterate over the remaining indices
                y = y + outputs[idx]
        return y
...
class Add(nn.Module):
    def __init__(self, weight=False):
        ...
        self.weight = weight
        if weight:
            self.w = nn.Parameter(-torch.arange(1.0, 2.0) / 2, requires_grad=True)  # Layer weights

    def forward(self, x):
        return x[0] + x[1] if not self.weight else x[0] * torch.sigmoid(self.w) + x[1] * (1 - torch.sigmoid(self.w))