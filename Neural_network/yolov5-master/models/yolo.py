...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            export.append(x[i])
            bs, random, ny, nx = x[i].shape  # x(bs,18, 2, 2) to x(bs,3,6,2,2)
...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # TODO: channel, gw, gd
            ...
            print("DATA")
            print(ch, f, args, m, n, na, no, nc, anchors)
            if f == -1:  # input from previous layer
                f = [i - 1]
            ...
            assert len(set(ch[x] for x in f)) == 1, f"Add layer inputs must have the same number of channels, got {ch[f]}"
            ch2 = ch[f[0]]
        else:
            c2 = ch[f]
