...
//    q7_t* cols_8b = (q7_t* )cols_8b_start;
    ...
    q7_t *cols_8b_start = (q7_t *)runtime_buf;
    int length = (input_x + 2) * (input_y + 2);

    for (int i = 0; i < length; i++) {
    	runtime_buf[i] = TN_MAX(runtime_buf[i], output_activation_min);
    	runtime_buf[i] = TN_MIN(runtime_buf[i], output_activation_max);
        cols_8b_start[i] = (q7_t) runtime_buf;
    }
    q7_t* cols_8b = (q7_t*) cols_8b_start;
        ...
        q7_t *cols_8b_iterptr, const uint16_t column_x, int channel_offset)
{
    ...
    for (i = 0; i < output_y; i++) {
        for (j = 0; j < output_x / 2; j++) {
            ...
            /* requantize */
            sum0 = (q31_t) roundf((float) sum0 * *scales);
            sum0 += output_offset;
            sum0 = TN_MAX(sum0, activation_min);
            sum0 = TN_MIN(sum0, activation_max);
            sum0 = roundf(sum0);
            output[(i * output_x + j * 2) * channel_offset] = (q7_t) sum0;

            sum1 = (q31_t) roundf((float) sum1 * *scales);
            sum1 += output_offset;
            sum1 = TN_MAX(sum1, activation_min);
            sum1 = TN_MIN(sum1, activation_max);
            sum1 = roundf(sum1);
            output[(i * output_x + (j * 2 + 1)) * channel_offset] = (q7_t) sum1;
        ...
        if (output_x & 1) {
            ...
            sum = (q31_t) roundf((float) sum * *scales);
            sum += output_offset;
            sum = TN_MAX(sum, activation_min);
            sum = TN_MIN(sum, activation_max);
            sum = roundf(sum);
            output[(i * output_x + output_x - 1) * channel_offset] = (q7_t) sum;
